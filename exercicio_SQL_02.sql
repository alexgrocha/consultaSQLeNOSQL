...
FROM filme
JOIN filme_categoria ON filme.id = filme_categoria.filme_id
JOIN categoria ON filme_categoria.categoria_id = categoria.id;

SELECT ator.nome, COUNT(filme_ator.filme_id) as numero_filmes
FROM ator
JOIN filme_ator ON ator.id = filme_ator.ator_id
GROUP BY ator.nome
...
FROM ator
JOIN filme_ator ON ator.id = filme_ator.ator_id
JOIN filme ON filme_ator.filme_id = filme.id
